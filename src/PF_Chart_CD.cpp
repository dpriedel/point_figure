...
PF_Chart::PF_Chart(const PF_Chart &rhs)
    : boxes_{rhs.boxes_}, columns_{rhs.columns_}, current_column_{rhs.current_column_}, symbol_{rhs.symbol_},
      ...
      last_checked_date_{rhs.last_checked_date_}, box_size_{rhs.box_size_}, reversal_boxes_{rhs.reversal_boxes_},
      y_min_{rhs.y_min_}, y_max_{rhs.y_max_}, current_direction_{rhs.current_direction_}, box_type_{rhs.box_type_},
      box_scale_{rhs.box_scale_}

{
    // now, the reason for doing this explicitly is to fix the column box pointers.

    std::ranges::for_each(columns_, [this](auto &col) { col.boxes_ = &this->boxes_; });
...
PF_Chart::PF_Chart(const std::string &symbol, DprDecimal::DDecQuad box_size, int32_t reversal_boxes,
    ...
    : boxes_{box_size, box_type, box_scale}, current_column_{&boxes_, reversal_boxes}, symbol_{symbol},
      box_size_{box_size}, reversal_boxes_{reversal_boxes}, box_type_{box_type}, box_scale_{box_scale}
...
std::string PF_Chart::ChartName(std::string_view suffix) const
{
    std::string chart_name =
        std::format("{}_{}{}X{}_{}.{}", symbol_, GetBoxsize(), (IsPercent() ? "%" : ""), GetReversalboxes(),
                    (box_scale_ == Boxes::BoxScale::e_linear ? "linear" : "percent"), suffix);
    return chart_name;
...
void PF_Chart::ConstructChartGraphAndWriteToFile(const fs::path &output_filename, Y_AxisFormat date_or_time) const
{
    ...
    std::ranges::for_each(x_axis_labels, [&x_labels](const auto &date) { x_labels.push_back(date.data()); });

    std::unique_ptr<XYChart> c = std::make_unique<XYChart>(1200, 1000);

    c->setPlotArea(50, 50, 1100, 850)->setGridColor(0xc0c0c0, 0xc0c0c0);

    c->addTitle(std::format("{}{} X {} for {}  {}.\nMost recent change: {}", box_size_.ToDouble(),
                            (IsPercent() ? "%" : ""), reversal_boxes_, symbol_, (IsPercent() ? "percent" : ""),
                            LocalDateTimeAsString(last_change_date_))
                    .c_str());
    ...
    auto *layer = c->addCandleStickLayer(DoubleArray(highData.data(), highData.size()),
                                         DoubleArray(lowData.data(), lowData.size()),
                                         DoubleArray(openData.data(), openData.size()),
...
void PF_Chart::FromJSON(const Json::Value &new_data)
{
    ...
    {
        throw std::invalid_argument{
            std::format("Invalid box_scale provided: {}. Must be 'linear' or 'percent'.", box_scale)};
    }
    ...
    columns_.clear();
    std::ranges::for_each(cols, [this](const auto &next_val) { this->columns_.emplace_back(&boxes_, next_val); });
...
{
    BOOST_ASSERT_MSG(
        the_data.size() > how_many_days,
        std::format("Not enough data provided. Need at least: {} values. Got {}.", how_many_days, the_data.size())
            .c_str());
    ...
    if (use_adjusted == UseAdjusted::e_Yes)
    {
        ...
        {
            high_minus_low = DprDecimal::DDecQuad{the_data[i]["adjHigh"].asString()} -
                             DprDecimal::DDecQuad{the_data[i]["adjLow"].asString()};
            high_minus_prev_close = (DprDecimal::DDecQuad{the_data[i]["adjHigh"].asString()} -
                                     DprDecimal::DDecQuad{the_data[i + 1]["adjClose"].asString()})
                                        .abs();
            low_minus_prev_close = (DprDecimal::DDecQuad{the_data[i]["adjLow"].asString()} -
                                    DprDecimal::DDecQuad{the_data[i + 1]["adjClose"].asString()})
                                       .abs();

            DprDecimal::DDecQuad max =
                DprDecimal::max(high_minus_low, DprDecimal::max(high_minus_prev_close, low_minus_prev_close));
    ...
    else
    {
        for (int i = 0; i < how_many_days; ++i)
        {
            high_minus_low = DprDecimal::DDecQuad{the_data[i]["high"].asString()} -
                             DprDecimal::DDecQuad{the_data[i]["low"].asString()};
            high_minus_prev_close = (DprDecimal::DDecQuad{the_data[i]["high"].asString()} -
                                     DprDecimal::DDecQuad{the_data[i + 1]["close"].asString()})
                                        .abs();
            low_minus_prev_close = (DprDecimal::DDecQuad{the_data[i]["low"].asString()} -
                                    DprDecimal::DDecQuad{the_data[i + 1]["close"].asString()})
                                       .abs();
