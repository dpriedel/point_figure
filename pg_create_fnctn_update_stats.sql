    ...
    IF all_or_symbol = '*' THEN
        ...
        FROM
            (
                ...
                WHERE t4.adjvolume > 0 AND t4.adjclose > 0.0 AND t4.date <= start_date AND t5.exchange IN ('BATS', 'NASDAQ', 'NYSE')
            ) AS t1
        WHERE
            rn <= rows_to_find
        ORDER BY
    ...
    ELSE
        ...
        FROM
            (
                ...
                WHERE t3.symbol = all_or_symbol AND t3.adjvolume > 0 AND t3.adjclose > 0.0 AND t3.date <= start_date
            ) AS t2
        WHERE
            rn <= rows_to_find
        ORDER BY
