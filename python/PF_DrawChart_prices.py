...
def DrawChart(the_data, ReversalBoxes, IsUp, StepBack, ChartTitle, ChartFileName, DateTimeFormat, ShowTrendLines,
    ...
    chart_data["Date"] = pd.to_datetime(chart_data["Date"], format="%f")
    ...
    prices["Time"] = pd.to_datetime(prices["the_time"], format="%Y-%m-%d %H:%M:%S%z")
    del prices["the_time"]
    ...
    fig = mpf.figure(style=s, figsize=(14, 14))
